...
!!
!!  In this module, it reads the functions and subroutines from other files and take the data in order to
!!  creating a new .dat files for operators to take the Neutron Flux from a reactor.
!!----------------------------------------------------------------------
...
!!
!!  This subroutine creates the files for the basic part. It requires the user to put necessary values.
!!----------------------------------------------------------------------
!! Output:
!!
...
subroutine read_input(depth, width, height, y_zero, x_min, x_max, x_step, n_grid, m_samples)
    ...
    height = read_real('width H')
    y_zero = read_real('y_zero')
    x_min = read_real('x minimum')
    x_max = read_real('x maximum')
    x_step = read_real('increment size for the x')
...
!!
!!  This function is to test whether the use put a valid values or not. It will have some warnings if the 
!!  number is invalid.
!!----------------------------------------------------------------------
!! Input:
!!
...
!!
!!      Same as the function read_real but this function is to check it's a integer or not.
!!----------------------------------------------------------------------
!! Input:
!!
!! name     character   A string with a brief description of the value being asked for
...
!! Describe what the function does
!!
!!      This subroutine writes all the information from users into the .dat files after the calculation.
!!----------------------------------------------------------------------
!! Input:
!!
!! depth        real        Depth of the rectangular nuclear reactor
...
!!
!!      Same as read_input but this subroutine is for the advanced part.
!!----------------------------------------------------------------------
!! Output:
!!
...
!-----------------------------------------------------------------------
 subroutine read_advanced_input(x_zero, r_min, r_max, r_step)
     ...
     print *, 'And we need some more informations for the advanced part...'

     ! Base this new subroutine on the read_input one
    x_zero = read_real('x_zero')
    r_min = read_real('r minimum')
    r_max = read_real('r maximum')
    r_step = read_real('increment size for the r')
...
!! Describe what the subroutine does
!!
!!      This subroutine write a new .dat file with the advanced part and informations.
!!----------------------------------------------------------------------
!! Input:
!! 
!! depth        real        Depth of the rectangular nuclear reactor
