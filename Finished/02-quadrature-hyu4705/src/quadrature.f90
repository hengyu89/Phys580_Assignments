...
!!
!!      This is the basic and common functions and method to calculate the integration with different methods
!!      like monte carlo; booles.
!!----------------------------------------------------------------------
...
!!
!!      This function use booles method to calculate the integration.
!! ----------------------------------------------------------------------
!! Input:
!!
...
real(dp) function booles_quadrature(fx, delta_x) result(s)
    ...
    if (mod(fx_size-1,4) /= 0) then
        print *, 'fx array size in booles_quadrature has to be a form that (N-1) is a multiple of 4'
        stop
    endif
...
!! Describe what the function does
!!
!!      This function is the part of booles_quadrature and it's the part inside the rimann sum.
!! ----------------------------------------------------------------------
!! Input:
!!
!! fx           real        Array containing the evaluated function
...
real(dp) function booles_rule(fx, delta_x) result(s)
    ...
    ! fx1 = ...
    ! ...
    s = ((2*delta_x)/(45)) * (7*fx4 + 32*fx3 + 12*fx2 + 32*fx1 + 7*fx0)
    ! s = ...
...
!! Describe what the function does
!!
!!      Compare to bools method, this sunroutine calculates the integration by Monte Carlo method.
!! ----------------------------------------------------------------------
!! Input:
!!
...
subroutine monte_carlo_quad(f, a, b, data, n_samples, s, sigma_s)
    ...
    real(dp), intent(out) :: s, sigma_s!, variance

    integer :: i, vector_size, volume
    real(dp), allocatable :: x_vector(:), fx(:), fx_squared(:)! ...you might need to declare other arrays here
    ...
    allocate(fx_squared(1:n_samples))
    ...
    do i=1,n_samples
        ...
        fx_squared(i) = f(x_vector,data)**2
    enddo
    ...
    s = volume * average(fx,n_samples)

    sigma_s = sqrt((average(fx_squared,n_samples) - average(fx,n_samples)**2)/n_samples)
    !sigma_s = sqrt((sum(fx_squared)-(sum(fx)/n_samples))/n_samples) * ((b(1) * b(2) * b(3)))
    !sigma_s = sqrt((sum(fx_squared)-(sum(fx)**2)/n_samples)/n_samples) * volume
...
real(dp) function average(fx, n_samples) result(ave)
    implicit none
    real(dp), intent(in) :: fx(1:)
    integer, intent(in) :: n_samples

    ave = sum(fx)/n_samples
