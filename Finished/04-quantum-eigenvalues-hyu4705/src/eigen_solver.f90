...
!! Heng Yu
!!
!! These subroutines and functions are used to compute the eigen problems 
!! of tridiagonal matrix.
!!
!! They will take two vectors as diagonal and off-diagonal as input. Then 
!! compute a vector as eigen values and an array as eigenvectors as output. 
!!----------------------------------------------------------------------
!! Included subroutines:
!!
!! solve_eigenproblem, tqli
!!----------------------------------------------------------------------
module eigen_solver
use types
implicit none
...
private
public solve_eigenproblem, tqli
...
!-----------------------------------------------------------------------
!! Dr. Navarro Perez and Heng Yu
!!
!! I took the reference of example that Dr. Navarro perez showed in the
!! class. Which returns the eigenvalues and eigenvectors of a symmetric 
!! tridiagonal matric.
!!
!!-----------------------------------------------------------------------
!! Input:
...
!! Output:
!!
!! lambdas(:)     real(dp)    Contains the eigenvalues of the tridiagonal matrix
!! vectors(:,:)   real(dp)    The k th column of vectors (i.e. vectors(:,k))  
!!                            returns the normalized eigenvector corresponding to lambdas(k) 
!-----------------------------------------------------------------------
subroutine solve_eigenproblem(d, e, lambdas, vectors)
    implicit none
    real(dp), intent(in) :: d(:), e(:)
    real(dp), intent(out), allocatable :: lambdas(:), vectors(:,:)
    ...
    allocate(lambdas(1:size(d)))
    allocate(vectors(1:size(d) , 1:size(d)))
