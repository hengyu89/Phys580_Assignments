...
real(dp) function rho_mn_int(length, center, width, n_max, n_samples, m, n) result(rho)
    ...
    !$omp parallel default(none) private(i_x, i_y, x, y, f_xy) &
    !$omp shared(n_samples, f_x, rho, m, n, l_x, l_y, delta_x, delta_y, center, width)
...
subroutine calculate_coefficients(length, rho_zero, center, width, n_max, n_samples, c_fourier)
    ...
    !$omp parallel default(none) private(m, n) &
    !$omp shared(n_max, rho_mn, norm, rho_zero, length, center, width, n_samples, c_fourier, l_x, l_y)
    !$omp do schedule(dynamic)
