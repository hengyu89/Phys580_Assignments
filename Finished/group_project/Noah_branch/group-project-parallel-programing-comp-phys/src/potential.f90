...
!-----------------------------------------------------------------------
real(dp) function rho_mn_int(length, center, width, n_max, m, n) result(rho)
    implicit none
    real(dp), intent(in) :: length(1:2), center(1:2), width(1:2)
    integer, intent(in) :: n_max, m, n
    real(dp), allocatable :: f_xy(:), f_x(:)
    real(dp) :: l_x, l_y, x, y, delta_x, delta_y
    integer :: i_y, i_x

    allocate(f_xy(1:n_max))
    allocate(f_x(1:n_max))
    ...
    delta_x = l_x/(real(n_max-1, kind=dp))
    delta_y = l_y/(real(n_max-1, kind=dp))
...
subroutine calculate_coefficients(length, rho_zero, center, width, n_max, c_fourier)
    ...
    !! omp & shared(length,rho_zero,normal,rho_coeff,center,width,l_x,l_y,r_x,r_y,c_fourier,n_max,num_threads)
    
    !! Get number of threads for output of time tests
    ...
    do m = 1, n_max
        do n = 1, n_max
            rho_mn = norm*rho_zero*rho_mn_int(length, center, width, n_max, m, n)
